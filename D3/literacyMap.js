(function() {
    d3.queue()
        ...
        .await(function(error, topoMain, topoKashmir) {
            ...
            // Map render
            var map     = districtMap(districts, disputed).width(1920).height(1200).scale(1900).propTag(filter);
            ...
            d3.selectAll("#select input[name=gender]").on("change", function() {
                ...
                colorCode(districts.features, filter);
                map     = districtMap(districts, disputed).width(1920).height(1200).scale(1900).propTag(filter);
                d3.select("#map").call(map);
            });
        });
}());
...
function colorCode(data, filter) {
    var color = d3.scale.threshold()
                  .domain([65, 72, 78, 85])
                  .range(["#a8ddb5", "#7bccc4", "#4eb3d3", "#2b8cbe", "#0868ac", "#084081", "#0E2E53"]);
